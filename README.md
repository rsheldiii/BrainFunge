...
Befunge is an esoteric programming language designed to be incredibly difficult to compile - it features (in the funge-93 specification) an instruction pointer that can move in 2 cardinal directions, an 80x25 grid of instructions, and the ability to modify its own code - "reflection". Once again the [Wikipedia](https://en.wikipedia.org/wiki/Befunge) article is a great place to learn more!
...
All Befunge programs start in the top left of the board and travel right unless directed otherwise. We immediately redirect the instruction pointer downwards, and pop two 0's onto the stack; these are the data and instruction pointer x values. Both values are actually offset by 1, but we just accomplish this by adding 1 to a copy of the pointer value whenever we need to reference something.
